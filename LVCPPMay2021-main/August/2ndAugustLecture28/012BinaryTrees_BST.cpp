	...
	if(key < root->data) {
		...
		if(root->left == NULL && root->right == NULL) {
			...
			if(root->left != NULL && root->right == NULL) {
				...
				root->left = del(root->left, replace->data);
				//I will delete replace also recursively
				 // or root->right = del(root->right, replace->data).This will be used if we replace 
				 //root value with min value n right subtree
				 //next step for this would have been
