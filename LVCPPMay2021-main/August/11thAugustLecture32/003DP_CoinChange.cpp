...
using namespace std;
//If the minimum coin denomination that u have is m,then the height of recursion tree will be n/m
//Total no of coins=k
//Total no. of sub-problems=k^n/m
//On each problem I spend O(k) time.So,TC=O(k*k^n/m)
...
//In worst case,we solve maximum n unique sub-problems.On each call we spend O(k) time
//TC=O(nk)
//SC=O(n+n/m)~O(n)
